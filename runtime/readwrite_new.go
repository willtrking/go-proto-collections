...
func writerChain(ctx context.Context, containers []*ReadWriteContainer, writer CollectionWriter, closeSignal *uint32, closeLock *sync.Mutex) ([]helpers.CollectionWriterResponse, bool, []WriterError) {
	...
	response, writeErr := writer.WriteNew(ctx)
...
func readWriteLevel(ctx context.Context, registry *CollectionRegistry, level []helpers.CollectionMessage,
	...
	if len(nextLevel) > 0 {
		...
		fmt.Printf("%# v", pretty.Formatter(nextLevel))
		//fmt.Printf("%# v", pretty.Formatter(newLevel))
...
func ReadWriteCollectionsNEW(ctx context.Context, registry *CollectionRegistry, topLevel []helpers.CollectionMessage) ([]helpers.CollectionMessage, map[string]map[string][]string) {
